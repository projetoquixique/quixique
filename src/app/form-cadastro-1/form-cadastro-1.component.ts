...
export class FormCadastro1Component implements OnInit {
  ...
  verificaCPF(){
    ...
    } else if (this.novoUsuario.cpf.length !== 11 || !(+(this.novoUsuario.cpf))) {
  ...
  verificaCep(){
  	...
  	} else if (this.novoUsuario.cep.length !== 9 || (/\d{5}[-]\\d{2}/.test(this.novoUsuario.cep))) {
  ...
  ngOnInit() {
    //tipo de usuário
    this.tipoDeUsuario = sessionStorage.getItem('cadastro');
